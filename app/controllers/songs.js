...
exports.findOnDiscogs = function(req, res) {
  ...
  var oauth = new OAuth(
    ...
    null,
    {
      'User-Agent': 'MusicCollection/0.0.1',
  ...
  oauth.getOAuthRequestToken(function(error, oauth_token, oauth_token_secret, results){
    if (error) util.puts('error :' + error);
      ...
      oauth.getOAuthAccessToken(oauth_token, oauth_token_secret, function(error, oauth_access_token, oauth_access_token_secret, results2) {
        ...
        util.puts('Requesting access token');
        oauth.getProtectedResource('http://api.discogs.com' + req.url, 'GET', oauth_access_token, oauth_access_token_secret,  function (error, data, response) {
  ...
  //require('http').get(options, function(response) {
    ...
    //});
  //});
  // , {headers: {'User-Agent': 'MusicCollection/0.0.1 +http://localhost:3000', 'Content-Type': 'application/json'}}
};

exports.coverDiscogs = function(req, res) {
  console.log(req.param('cover'));
  var OAuth = require('oauth').OAuth;
  var oauth = new OAuth(
    'http://api.discogs.com/oauth/request_token',
    'http://api.discogs.com/oauth/access_token',
  ...
  oauth.getOAuthRequestToken(function(error, oauth_token, oauth_token_secret, results){
    if (error) util.puts('error :' + error);
      ...
      oauth.getOAuthAccessToken(oauth_token, oauth_token_secret, function(error, oauth_access_token, oauth_access_token_secret, results2) {
        ...
        util.puts('accesstoken results :' + util.inspect(results2));
        util.puts('Requesting access token');
        oauth.getProtectedResource(req.param('cover'), 'GET', oauth_access_token, oauth_access_token_secret,  function (error, data, response) {
          util.puts(data);
          res.send(data);
        });
      });
